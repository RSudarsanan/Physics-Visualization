...
var mode = 'light'; 
var tools_withList = ['Light_Source_','mirror_', 'refractor_'];
var tools_inList = ['Ray', 'Point_Source', 'Beam','mirror', 'arcmirror', 'idealmirror', 'lens', 'refractor'];
var modes = ['light'];
...
function init_page_headers() {
  ...
  document.getElementById('tool_Ray').dataset['n'] = getMsg('toolname_Ray');
  document.getElementById('tool_Point_Source').dataset['n'] = getMsg('toolname_Point_Source');
  document.getElementById('tool_Beam').dataset['n'] = getMsg('toolname_Beam');
...
function shootWaitingRays(){
  ...
  for (var i = 0; i < objs.length; i++){
    Constructs[objs[i].type].draw(objs[i], canvas, true); //畫出objs[i]
  }
  setTimeout(draw_, 10);
}
...
"toolname_Point_Source": {
  "message": "Point_source"
},
"toolname_Beam": {
  "message": "Beam"
},
