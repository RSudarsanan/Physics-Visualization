...
/***/ function(module, exports, __webpack_require__) {
	...
	Gui.create = function (engine, runner, render) {
	  ...
	  var gui = {
	    ...
	    stiffness: 0.5,
	    damping: 0,
	    rod:{Ax:0, Ay:0, Bx: 0, By:0, BodyA:-1, BodyB:-1}
	  };
	...
	var _initDatGui = function _initDatGui(gui) {
	  ...
	  var funcs = {
	    ...
	    run: function run(){
			...
			for(var i=0;i<index.length;i++){
				engine.world.bodies[index[i]].force=force[i]; 
			}
			force=[];
			index=[];
	    	static_status=false; 
	  ...
	  if (engine) {
	    ...
	    controls.add(gui, 'xCoordinate', 10, 790).step(1);
	    controls.add(gui, 'yCoordinate', 10, 515).step(1);
	    controls.add(gui, 'xImpulse', -1.00, 1.00).step(0.01);
	    controls.add(gui, 'yImpulse', -1.00, 1.00).step(0.01);
	    ...
	    var springs = datGui.addFolder('Springs');
	    var pointA=springs.addFolder('pointA');
	    pointA.add(gui, 'Ax', 10, 515).step(1);
	    pointA.add(gui, 'Ay', 10, 515).step(1);
	    pointA.open();
	    var pointB=springs.addFolder('pointB');
	    pointB.add(gui, 'Bx', 10, 515).step(1);
	    pointB.add(gui, 'By', 10, 515).step(1);
	    pointB.open();
	    springs.add(gui, 'BodyA', -1, 10).step(1);
	    springs.add(gui, 'BodyB', -1, 10).step(1);
	    springs.add(gui, 'damping', 0, 1).step(0.001);
		springs.add(gui, 'stiffness', 0.001, 1).step(0.001);
		springs.add(funcs, 'AddSpring');

		var rods = datGui.addFolder('Rods');
	    var pointA=rods.addFolder('pointA');
	    pointA.add(gui.rod, 'Ax', -20, 515).step(1);
	    pointA.add(gui.rod, 'Ay', -20, 515).step(1);
	    pointA.open();
	    var pointB=rods.addFolder('pointB');
	    pointB.add(gui.rod, 'Bx', -20, 515).step(1);
	    pointB.add(gui.rod, 'By', -20, 515).step(1);
	    pointB.open();
...
//			damping: gui.damping,
		...
		if(gui.rod.BodyA != -1)
		{
			options.bodyA = engine.world.bodies[gui.rod.BodyA];
		}
		if(gui.rod.BodyB != -1)
		{
			options.bodyB = engine.world.bodies[gui.rod.BodyB];
		}
		var rod = Matter.Constraint.create(options);
	...
	var _addSpring = function _addSpring(gui) {
		var engine = gui.engine;
		var options = {
			pointA: {x:gui.Ax, y:gui.Ay},
			pointB: {x:gui.Bx, y:gui.By},
			stiffness: gui.stiffness,
			damping: gui.damping,
		...
		if(gui.BodyA != -1)
		{
			options.bodyA = engine.world.bodies[gui.BodyA];
		}
		if(gui.BodyB != -1)
		{
			options.bodyB = engine.world.bodies[gui.BodyB];
		}
		var spring = Matter.Constraint.create(options);
	...
	var _addBody = function _addBody(gui) {
	  ...
	  var options = {
	    ...
	    isStatic: false,
	    force: {x: gui.xImpulse, y: gui.yImpulse }
	  };
	  ...
	  for (var i = 0; i < gui.amount; i++) {
	    var temp=Bodies.polygon(gui.xCoordinate, gui.yCoordinate, gui.sides, gui.size, options);
	    World.add(engine.world, temp);
	    console.log(temp);
	    engine.world.bodies[engine.world.bodies.length-1].isStatic=static_status;
	   	if(static_status){
